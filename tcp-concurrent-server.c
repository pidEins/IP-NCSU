...
void * serverthread(void * parm)
{
   int tsd, tvisits;
   int tot, n;
   char message_to_archive[MAX_SIZE+1]; /* buffer for data to receive */
   ...
   tot = 0;
   do {
	tot += (n = recv (tsd, message_to_archive+tot, MAX_SIZE-tot, 0));
        if (n < 0) {
   ...
   /* send the strlen(buf) + 1 bytes because strlen will not include the \0 
   while calculating string size append the \0 at the end of the string.
   So to send the string will NULL terminating character , we need to send
   strlen(buf) + bytes */
   send(tsd,buf,strlen(buf) + 1,0);
